...
from bot_events_commands import bot_app
...
class TestSlashCommandHandling(unittest.TestCase): 
    ...
    def test_compute_time_format(self):
        ...
        self.assertEqual(bot_events_commands.compute_time_format("1s"), 1)
        #tests if minutes returns correct second value.
        self.assertEqual(bot_events_commands.compute_time_format("60m"),3600)
        #tests if days returns correct second value.
        self.assertEqual(bot_events_commands.compute_time_format("1d"),86400)
        #tests if multiple units days = d, second = s, minute = m, can return successfully.
        self.assertEqual(bot_events_commands.compute_time_format("1d30m"),88200)
        #tests whether extra space can return successfully.
        self.assertEqual(bot_events_commands.compute_time_format("1d 30m"),88200)
        #tests if multipe ordering of the different time units can return successfully.
        self.assertEqual(bot_events_commands.compute_time_format("30m1d"),88200)
        self.assertEqual(bot_events_commands.compute_time_format("30my1d"),88200)
        #tests if an unknown letter/unit is entered that return value defaults to number of seconds
        #per minute
        self.assertEqual(bot_events_commands.compute_time_format("60q"),3600)
        #tests if absence of letter/unit return value defaults to number of seconds per minute
        self.assertEqual(bot_events_commands.compute_time_format("60"),3600)
        ...
        self.assertEqual(bot_events_commands.compute_time_format("30 30"),3600)
    ...
    def test_valid_handle_join_class_invocation(self):
        ...
        actual_response = bot_events_commands.handle_join_class_invocation(valid_payload)
        self.assertTrue(self.same_responses(actual_response, self.valid_payload_response))
        valid_payload = self.make_non_defective_payload(self.join_class_command, 'csmc 22001')
        actual_response = bot_events_commands.handle_join_class_invocation(valid_payload)
        self.assertTrue(self.same_responses(actual_response, self.valid_payload_response))
    ...
    def test_check_valid_slash_command_payload(self):
        valid_payload = self.make_non_defective_payload(self.disable_activity_warnings_command, '2h')
        actual_response = bot_events_commands.check_valid_slash_command_payload(valid_payload, valid_payload["team_id"], valid_payload["token"])
        self.assertTrue(actual_response)
        invalid_payload = self.make_defective_payload(self.disable_activity_warnings_command, '2h')
        actual_response = bot_events_commands.check_valid_slash_command_payload(invalid_payload, "not-the-team_id", "not-the-token")
...
class TestCheckingPayload(unittest.TestCase):
    ...
    def test_valid_payload(self):
        ...
        team_id = "T061EG9RZ"
        token = "z26uFbvR1xHJEdHE1OQiO6t8"
        self.assertTrue(bot_events_commands.check_valid_event_payload(self.payload, team_id, token))
        self.assertFalse(bot_events_commands.check_valid_event_payload(self.bad_payload, team_id, token))
        self.assertFalse(bot_events_commands.check_valid_event_payload(self.bad_payload2, team_id, token))
        self.assertFalse(bot_events_commands.check_valid_event_payload(self.bad_payload3, team_id, token))
...
class TestHandlingWorkspace(unittest.TestCase):
    ...
    def test_channel_creation(self):
        ...
        self.assertTrue(bot_events_commands.check_id(self.payload, bot_id1))
        self.assertFalse(bot_events_commands.check_id(self.payload, bot_id2))
